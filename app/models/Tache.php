...
class Tache {
    ...
    public function find($id) {
        $stmt = $this->db->prepare("SELECT * FROM bao_tache WHERE id_tache = ?");
        $stmt->execute([$id]);
    ...
    public function create($data) {
        $stmt = $this->db->prepare("INSERT INTO bao_tache (id_employe_poste, nom_tache, description) VALUES (?, ?, ?)");
        return $stmt->execute([$data['id_employe_poste'], $data['nom_tache'], $data['description']]);
    }

    // Modifier une tâche
    public function update($id, $data) {
        $stmt = $this->db->prepare("UPDATE bao_tache SET id_employe_poste=?, nom_tache=?, description=? WHERE id_tache=?");
        return $stmt->execute([$data['id_employe_poste'], $data['nom_tache'], $data['description'], $id]);
    }

    // Supprimer une tâche
    public function delete($id) {
        $stmt = $this->db->prepare("DELETE FROM bao_tache WHERE id_tache=?");
        return $stmt->execute([$id]);
    }

    // Récupérer les postes (rôles)
    public function getPostes() {
        return $this->db->query("SELECT * FROM bao_employe_poste")->fetchAll(PDO::FETCH_ASSOC);
    }

    // Récupérer les tâches filtrées par poste
    public function getByPoste($id_employe_poste) {
        $stmt = $this->db->prepare("SELECT * FROM bao_tache WHERE id_employe_poste = ?");
    ...
    public function getEmploye($id_employe) {
        $stmt = $this->db->prepare("SELECT * FROM bao_employe WHERE id_employe = ?");
    ...
    public function getEmployesActifs() {
        return $this->db->query("SELECT * FROM bao_employe WHERE statut = 'actif'")->fetchAll(PDO::FETCH_ASSOC);
    }

    // Assigner une tâche à un employé
    public function assignTache($data) {
        $stmt = $this->db->prepare("INSERT INTO bao_tache_employe (id_tache, id_employe, date_attribution, statut, date_echeance) VALUES (?, ?, ?, ?, ?)");
        return $stmt->execute([
            $data['id_tache'],
            $data['id_employe'],
            date('Y-m-d'),
    ...
    public function setTachesDone($ids) {
        $stmt = $this->db->prepare('UPDATE bao_tache_employe SET statut = ? WHERE id_tache_employe = ?');
        foreach ($ids as $id) {
